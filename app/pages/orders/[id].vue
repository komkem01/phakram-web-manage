...
const { isLoading, isItemsLoading, errorMessage, successMessage, orderItems, orderTimeline, itemsPaginate, fetchOrderById, fetchOrderItems, fetchOrderTimeline, updateOrderStatus } = useSystemOrders()
...
const canSubmitStatus = computed(() => Boolean(order.value && statusForm.value && statusForm.value !== order.value.status && !isStatusSubmitting.value))
      ...
      <div class="card-section" v-if="order">
        ...
        <div class="status-form-row">
          <div class="form-group status-form-group">
            ...
            <select v-model="statusForm" :disabled="isStatusSubmitting || statusOptions.length === 0">
        ...
        <div class="info-grid">
          <div class="info-item"><span class="label">รหัสคำสั่งซื้อ</span><span class="value">{{ order.order_no || '-' }}</span></div>
          ...
          <div class="info-item"><span class="label">Member ID</span><span class="value">{{ order.member_id || '-' }}</span></div>
          <div class="info-item"><span class="label">Payment ID</span><span class="value">{{ order.payment_id || '-' }}</span></div>
          <div class="info-item"><span class="label">Address ID</span><span class="value">{{ order.address_id || '-' }}</span></div>
            ...
            <div class="timeline-content">
              <div class="timeline-head">
                <span class="timeline-title">
                  {{ item.from_status ? statusLabel(item.from_status) : '-' }}
                  →
                  {{ item.to_status ? statusLabel(item.to_status) : '-' }}
                </span>
              ...
              <p class="timeline-meta">{{ formatDateTime(item.created_at) }} · ผู้กระทำ: {{ item.action_by || '-' }}</p>
            </div>
          </div>
        </div>
      </div>
