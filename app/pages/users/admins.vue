...
async function load(page = 1) {
  ...
  await fetchUsers({ page, size: pageSize.value, role: 'admin', sort_by: 'created_at', order_by: 'desc' })
}
...
async function handleSubmit() {
  ...
  if (!form.prefix_id || !form.gender_id || !firstnameTh || !lastnameTh || !firstnameEn || !lastnameEn || !phone || !form.tier_id || !form.status_id) {
  ...
  if (editingId.value) {
    ...
    if (!email || !password) {
      ...
      showToast('warning', 'กรุณากรอกอีเมลและรหัสผ่านสำหรับการสร้างแอดมิน')
      return
    }
...
<template>
  <div class="settings-page">
    ...
    <div class="page-header">
      ...
      <p class="page-subtitle">เพิ่ม/แก้ไข/ลบ ผู้ใช้งานที่เป็นแอดมิน</p>
    </div>

    <div class="content-card">
      <form class="card-section" @submit.prevent="handleSubmit">
        <h3 class="section-title">{{ isEditing ? 'แก้ไขแอดมิน' : 'เพิ่มแอดมิน' }}</h3>
        <p class="hint-text">แอดมินสมัครให้แอดมิน: ต้องกำหนดระดับสมาชิก (Tier) และสถานะ (Status)</p>
        ...
        <div class="form-grid">
          ...
          <div class="form-group"><label>ระดับสมาชิก (Tier)</label><select v-model="form.tier_id" :disabled="isSubmitting"><option value="">เลือก Tier</option><option v-for="item in tierApi.tiers.value" :key="item.id" :value="item.id">{{ item.name_th }}</option></select></div>
          <div class="form-group"><label>สถานะ (Status)</label><select v-model="form.status_id" :disabled="isSubmitting"><option value="">เลือก Status</option><option v-for="item in statusApi.statuses.value" :key="item.id" :value="item.id">{{ item.name_th }}</option></select></div>
      ...
      <div class="card-section">
        <div class="table-header">
          <h3 class="section-title">รายการแอดมิน</h3>
          ...
          <table class="data-table">
            ...
            <tbody>
              ...
              <tr v-for="item in users" :key="item.id">
                ...
                <td>{{ item.phone || '-' }}</td>
                <td><span class="badge badge-admin">แอดมิน</span></td>
    ...
    <div v-if="isDeleteModalOpen" class="confirm-modal-overlay" @click="closeDeleteModal">
      <div class="confirm-modal" @click.stop>
        ...
        <p class="confirm-modal-text">คุณต้องการลบผู้ดูแลระบบนี้ใช่หรือไม่?</p>
