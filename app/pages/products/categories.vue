...
import type { CategoryPayload } from '~/types/settings'

definePageMeta({ layout: 'dashboard' })

const categoryApi = useSystemCategories()
const parentApi = useSystemCategories()

const { isLoading, errorMessage, successMessage, categories, paginate, fetchCategories, createCategory, updateCategory, deleteCategory } = categoryApi

const form = reactive<CategoryPayload>({ parent_id: null, name_th: '', name_en: '', is_active: true })
...
const parentOptions = computed(() => parentApi.categories.value)
const parentMap = computed(() => parentOptions.value.reduce<Record<string, string>>((acc, item) => { acc[item.id] = item.name_th; return acc }, {}))
...
async function load(page = 1) {
  ...
  await fetchCategories({ page, size: pageSize.value, sort_by: 'created_at', order_by: 'desc' })
}

async function loadParentOptions() {
  await parentApi.fetchCategories({ page: 1, size: 500, sort_by: 'name_th', order_by: 'asc' })
}
...
async function handleSubmit() {
  ...
  const success = editingId.value ? await updateCategory(editingId.value, payload) : await createCategory(payload)
  isSubmitting.value = false
  if (!success) return
  resetForm()
  await Promise.all([load(currentPage.value), loadParentOptions()])
}
...
async function handleDelete(id: string) {
  ...
  if (categories.value.length === 1 && currentPage.value > 1) {
    await Promise.all([load(currentPage.value - 1), loadParentOptions()])
    return
  }
  await Promise.all([load(currentPage.value), loadParentOptions()])
}
...
onMounted(async () => {
  await Promise.all([load(1), loadParentOptions()])
})
...
<template>
  <div class="settings-page">
    ...
    <div class="page-header">
      <h1 class="page-title">จัดการสินค้า - หมวดหมู่สินค้า</h1>
      <p class="page-subtitle">จัดการข้อมูลหมวดหมู่สินค้า</p>
    </div>

    <div class="content-card">
      <form class="card-section" @submit.prevent="handleSubmit">
        <h3 class="section-title">{{ isEditing ? 'แก้ไขหมวดหมู่สินค้า' : 'เพิ่มหมวดหมู่สินค้า' }}</h3>

        <div class="form-grid">
          <div class="form-group">
            <label for="parent_id">หมวดหมู่หลัก</label>
            <select id="parent_id" v-model="form.parent_id" :disabled="isSubmitting">
              <option :value="null">ไม่มีหมวดหมู่หลัก</option>
              <option v-for="item in parentOptions" :key="item.id" :value="item.id">{{ item.name_th }}</option>
            </select>
          </div>
          <div class="form-group">
            <label for="name_th">ชื่อหมวดหมู่ (ไทย)</label>
            <input id="name_th" v-model="form.name_th" type="text" :disabled="isSubmitting" required>
          </div>
          <div class="form-group">
            <label for="name_en">ชื่อหมวดหมู่ (อังกฤษ)</label>
      ...
      <div class="card-section">
        <div class="table-header">
          <h3 class="section-title">รายการหมวดหมู่สินค้า</h3>
        ...
        <div v-else class="table-wrapper">
          <table class="data-table">
            <thead><tr><th>ชื่อ (ไทย)</th><th>ชื่อ (อังกฤษ)</th><th>หมวดหลัก</th><th>สถานะ</th><th class="actions-col">จัดการ</th></tr></thead>
            <tbody>
              <tr v-if="categories.length === 0"><td colspan="5" class="empty-cell">ไม่พบข้อมูล</td></tr>
              <tr v-for="item in categories" :key="item.id">
                <td>{{ item.name_th }}</td>
                <td>{{ item.name_en }}</td>
                <td>{{ item.parent_id ? (parentMap[item.parent_id] || '-') : '-' }}</td>
    ...
    <div v-if="isDeleteModalOpen" class="confirm-modal-overlay" @click="closeDeleteModal">
      <div class="confirm-modal" @click.stop>
        ...
        <p class="confirm-modal-text">คุณต้องการลบข้อมูลหมวดหมู่สินค้านี้ใช่หรือไม่?</p>
