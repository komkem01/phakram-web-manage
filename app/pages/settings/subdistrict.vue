...
import type { SubDistrictPayload } from '~/types/settings'

definePageMeta({ layout: 'dashboard' })

const { isLoading, errorMessage, successMessage, subDistricts, paginate, fetchSubDistricts, createSubDistrict, updateSubDistrict, deleteSubDistrict } = useSystemSubDistricts()
const { districts, fetchDistricts } = useSystemDistricts()

const form = reactive<SubDistrictPayload>({ district_id: '', name: '', is_active: true })
...
function startEditRow(id: string) {
  const item = subDistricts.value.find((subDistrict) => subDistrict.id === id)
  if (!item) return
...
async function load(page = 1) {
  ...
  await fetchSubDistricts({ page, size: pageSize.value, sort_by: 'created_at', order_by: 'desc' })
}
...
async function handleSubmit() {
  ...
  const payload: SubDistrictPayload = { district_id: form.district_id, name: form.name.trim(), is_active: form.is_active }
  const success = editingId.value ? await updateSubDistrict(editingId.value, payload) : await createSubDistrict(payload)
...
async function handleDelete(id: string) {
  const success = await deleteSubDistrict(id)
  if (!success) return
  if (subDistricts.value.length === 1 && currentPage.value > 1) {
...
<template>
  <div class="settings-page">
    ...
    <div class="page-header">
      <h1 class="page-title">ตั้งค่าพื้นฐาน - ตำบล</h1>
      <p class="page-subtitle">จัดการข้อมูลตำบล</p>
    </div>

    <div class="content-card">
      <form class="card-section" @submit.prevent="handleSubmit">
        <h3 class="section-title">{{ isEditing ? 'แก้ไขตำบล' : 'เพิ่มตำบล' }}</h3>
      ...
      <div class="card-section">
        <div class="table-header">
          <h3 class="section-title">รายการตำบล</h3>
        ...
        <div v-else class="table-wrapper">
          <table class="data-table">
            <thead><tr><th>ชื่อตำบล</th><th>อำเภอ</th><th>สถานะ</th><th class="actions-col">จัดการ</th></tr></thead>
            <tbody>
              <tr v-if="subDistricts.length === 0"><td colspan="4" class="empty-cell">ไม่พบข้อมูล</td></tr>
              <tr v-for="item in subDistricts" :key="item.id">
                <td>{{ item.name }}</td>
                <td>{{ districtMap[item.district_id] || '-' }}</td>
    ...
    <div v-if="isDeleteModalOpen" class="confirm-modal-overlay" @click="closeDeleteModal">
      <div class="confirm-modal" @click.stop>
        ...
        <p class="confirm-modal-text">คุณต้องการลบข้อมูลตำบลนี้ใช่หรือไม่?</p>
