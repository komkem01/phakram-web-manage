...
import type { ProvincePayload } from '~/types/settings'
...
function startEditRow(id: string) {
  const item = provinces.value.find((province) => province.id === id)
  if (!item) {
    return
  }
...
async function handleSubmit() {
  ...
  const success = editingId.value
    ? await updateProvince(editingId.value, payload)
...
async function handleDelete(id: string) {
  const success = await deleteProvince(id)
  if (!success) {
    return
  }

  if (provinces.value.length === 1 && currentPage.value > 1) {
...
<template>
  <div class="settings-page">
    ...
    <div class="page-header">
      <h1 class="page-title">ตั้งค่าพื้นฐาน - จังหวัด</h1>
      <p class="page-subtitle">จัดการข้อมูลจังหวัด</p>
    </div>

    <div class="content-card">
      <form class="card-section" @submit.prevent="handleSubmit">
        <h3 class="section-title">{{ isEditing ? 'แก้ไขจังหวัด' : 'เพิ่มจังหวัด' }}</h3>
        <div class="form-group">
          <label for="name">ชื่อจังหวัด</label>
      ...
      <div class="card-section">
        <div class="table-header">
          <h3 class="section-title">รายการจังหวัด</h3>
        ...
        <div v-else class="table-wrapper">
          <table class="data-table">
            <thead><tr><th>ชื่อจังหวัด</th><th>สถานะ</th><th class="actions-col">จัดการ</th></tr></thead>
            <tbody>
              <tr v-if="provinces.length === 0"><td colspan="3" class="empty-cell">ไม่พบข้อมูล</td></tr>
    ...
    <div v-if="isDeleteModalOpen" class="confirm-modal-overlay" @click="closeDeleteModal">
      <div class="confirm-modal" @click.stop>
        ...
        <p class="confirm-modal-text">คุณต้องการลบข้อมูลจังหวัดนี้ใช่หรือไม่?</p>
...
.form-group input { height: 42px; border: 1px solid #d1d5db; border-radius: 8px; padding: 0 12px; font-size: 14px; outline: none; }
