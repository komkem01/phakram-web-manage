...
import type { TierPayload } from '~/types/settings'

definePageMeta({ layout: 'dashboard' })

const { isLoading, errorMessage, successMessage, tiers, paginate, fetchTiers, createTier, updateTier, deleteTier } = useSystemTiers()

const form = reactive<TierPayload>({ name_th: '', name_en: '', min_spending: '', is_active: true, discount_rate: '' })
...
function startEditRow(id: string) {
  const item = tiers.value.find((tier) => tier.id === id)
  if (!item) return

  editingId.value = item.id
  form.name_th = item.name_th
  form.name_en = item.name_en
  form.min_spending = String(item.min_spending)
...
async function load(page = 1) {
  ...
  await fetchTiers({ page, size: pageSize.value, sort_by: 'created_at', order_by: 'desc' })
}
...
async function handleSubmit() {
  ...
  const minSpending = String(form.min_spending ?? '').trim()
  const discountRate = String(form.discount_rate ?? '').trim()

  if (!nameTh || !nameEn || !minSpending || !discountRate) {
  ...
  const success = editingId.value ? await updateTier(editingId.value, payload) : await createTier(payload)
...
async function handleDelete(id: string) {
  ...
  if (!success) return
  if (tiers.value.length === 1 && currentPage.value > 1) {
...
<template>
  <div class="settings-page">
    ...
    <div class="page-header">
      <h1 class="page-title">ตั้งค่าพื้นฐาน - ระดับสมาชิก</h1>
      <p class="page-subtitle">จัดการข้อมูลระดับสมาชิก</p>
    </div>

    <div class="content-card">
      <form class="card-section" @submit.prevent="handleSubmit">
        <h3 class="section-title">{{ isEditing ? 'แก้ไขระดับสมาชิก' : 'เพิ่มระดับสมาชิก' }}</h3>
        ...
        <div class="form-grid">
          ...
          <div class="form-group">
            <label for="min_spending">ยอดใช้จ่ายขั้นต่ำ</label>
            <input id="min_spending" v-model="form.min_spending" type="number" min="0" step="0.01" :disabled="isSubmitting" required>
          </div>
          <div class="form-group">
            <label for="discount_rate">ส่วนลด (%)</label>
            <input id="discount_rate" v-model="form.discount_rate" type="number" min="0" step="0.01" :disabled="isSubmitting" required>
          </div>
        </div>
      ...
      <div class="card-section">
        <div class="table-header">
          <h3 class="section-title">รายการระดับสมาชิก</h3>
        ...
        <div v-else class="table-wrapper">
          <table class="data-table">
            <thead><tr><th>ชื่อ (ไทย)</th><th>ชื่อ (อังกฤษ)</th><th>ยอดใช้จ่ายขั้นต่ำ</th><th>ส่วนลด (%)</th><th>สถานะ</th><th class="actions-col">จัดการ</th></tr></thead>
            <tbody>
              <tr v-if="tiers.length === 0"><td colspan="6" class="empty-cell">ไม่พบข้อมูล</td></tr>
              <tr v-for="item in tiers" :key="item.id">
                <td>{{ item.name_th }}</td>
                <td>{{ item.name_en }}</td>
                <td>{{ item.min_spending }}</td>
    ...
    <div v-if="isDeleteModalOpen" class="confirm-modal-overlay" @click="closeDeleteModal">
      <div class="confirm-modal" @click.stop>
        ...
        <p class="confirm-modal-text">คุณต้องการลบข้อมูลระดับสมาชิกนี้ใช่หรือไม่?</p>
