...
import type { ZipcodePayload } from '~/types/settings'

definePageMeta({ layout: 'dashboard' })

const { isLoading, errorMessage, successMessage, zipcodes, paginate, fetchZipcodes, createZipcode, updateZipcode, deleteZipcode } = useSystemZipcodes()
const { subDistricts, fetchSubDistricts } = useSystemSubDistricts()

const form = reactive<ZipcodePayload>({ sub_districts_id: '', name: '', is_active: true })
...
function startEditRow(id: string) {
  const item = zipcodes.value.find((zipcode) => zipcode.id === id)
  if (!item) return
...
async function load(page = 1) {
  ...
  await fetchZipcodes({ page, size: pageSize.value, sort_by: 'created_at', order_by: 'desc' })
}
...
async function handleSubmit() {
  if (!form.name.trim() || !form.sub_districts_id) {
    showToast('warning', 'กรุณากรอกข้อมูลให้ครบถ้วน')
    return
  }

  isSubmitting.value = true
  const payload: ZipcodePayload = { sub_districts_id: form.sub_districts_id, name: form.name.trim(), is_active: form.is_active }
  const success = editingId.value ? await updateZipcode(editingId.value, payload) : await createZipcode(payload)
...
async function handleDelete(id: string) {
  const success = await deleteZipcode(id)
  if (!success) return
  if (zipcodes.value.length === 1 && currentPage.value > 1) {
...
<template>
  <div class="settings-page">
    ...
    <div class="page-header">
      <h1 class="page-title">ตั้งค่าพื้นฐาน - รหัสไปรษณีย์</h1>
      <p class="page-subtitle">จัดการข้อมูลรหัสไปรษณีย์</p>
    </div>

    <div class="content-card">
      <form class="card-section" @submit.prevent="handleSubmit">
        <h3 class="section-title">{{ isEditing ? 'แก้ไขรหัสไปรษณีย์' : 'เพิ่มรหัสไปรษณีย์' }}</h3>
      ...
      <div class="card-section">
        <div class="table-header">
          <h3 class="section-title">รายการรหัสไปรษณีย์</h3>
        ...
        <div v-else class="table-wrapper">
          <table class="data-table">
            <thead><tr><th>รหัสไปรษณีย์</th><th>ตำบล</th><th>สถานะ</th><th class="actions-col">จัดการ</th></tr></thead>
            <tbody>
              <tr v-if="zipcodes.length === 0"><td colspan="4" class="empty-cell">ไม่พบข้อมูล</td></tr>
              <tr v-for="item in zipcodes" :key="item.id">
                <td>{{ item.name }}</td>
                <td>{{ subDistrictMap[item.sub_districts_id] || '-' }}</td>
    ...
    <div v-if="isDeleteModalOpen" class="confirm-modal-overlay" @click="closeDeleteModal">
      <div class="confirm-modal" @click.stop>
        ...
        <p class="confirm-modal-text">คุณต้องการลบข้อมูลรหัสไปรษณีย์นี้ใช่หรือไม่?</p>
