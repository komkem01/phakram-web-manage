...
import type { DistrictPayload } from '~/types/settings'

definePageMeta({ layout: 'dashboard' })

const { isLoading, errorMessage, successMessage, districts, paginate, fetchDistricts, createDistrict, updateDistrict, deleteDistrict } = useSystemDistricts()
const { provinces, fetchProvinces } = useSystemProvinces()

const form = reactive<DistrictPayload>({ province_id: '', name: '', is_active: true })
...
const provinceMap = computed(() => provinces.value.reduce<Record<string, string>>((acc, item) => { acc[item.id] = item.name; return acc }, {}))
const provinceSearch = ref('')

function syncProvinceIdFromSearch() {
  const keyword = provinceSearch.value.trim().toLowerCase()
  if (!keyword) {
    form.province_id = ''
    return
  }
  const matched = provinces.value.find((item) => item.name.trim().toLowerCase() === keyword)
  form.province_id = matched?.id || ''
}
...
function resetForm() {
  ...
  provinceSearch.value = provinces.value[0]?.name || ''
  form.is_active = true
}

function startEditRow(id: string) {
  const item = districts.value.find((district) => district.id === id)
  if (!item) return
  editingId.value = item.id
  form.name = item.name
  form.province_id = item.province_id
  provinceSearch.value = provinceMap.value[item.province_id] || ''
...
async function load(page = 1) {
  ...
  await fetchDistricts({ page, size: pageSize.value, sort_by: 'created_at', order_by: 'desc' })
}
...
async function loadProvinces() {
  await fetchProvinces({ page: 1, size: 100, sort_by: 'created_at', order_by: 'desc' })
  if (!form.province_id && provinces.value.length > 0) {
    form.province_id = provinces.value[0].id
    provinceSearch.value = provinces.value[0].name
    return
  }
  provinceSearch.value = provinceMap.value[form.province_id] || ''
}
...
async function handleSubmit() {
  ...
  const payload: DistrictPayload = { name: form.name.trim(), province_id: form.province_id, is_active: form.is_active }
  const success = editingId.value ? await updateDistrict(editingId.value, payload) : await createDistrict(payload)
...
async function handleDelete(id: string) {
  const success = await deleteDistrict(id)
  if (!success) return
  if (districts.value.length === 1 && currentPage.value > 1) {
...
<template>
  <div class="settings-page">
    ...
    <div class="page-header">
      <h1 class="page-title">ตั้งค่าพื้นฐาน - อำเภอ</h1>
      <p class="page-subtitle">จัดการข้อมูลอำเภอ</p>
    </div>

    <div class="content-card">
      <form class="card-section" @submit.prevent="handleSubmit">
        <h3 class="section-title">{{ isEditing ? 'แก้ไขอำเภอ' : 'เพิ่มอำเภอ' }}</h3>

        <div class="form-grid">
          <div class="form-group">
            <label for="name">ชื่ออำเภอ</label>
            <input id="name" v-model="form.name" type="text" :disabled="isSubmitting" required>
          </div>
          <div class="form-group">
            ...
            <input
              ...
              @input="syncProvinceIdFromSearch"
              @change="syncProvinceIdFromSearch"
              required
            >
            <datalist id="province-options">
              <option v-for="item in provinces" :key="item.id" :value="item.name" />
            </datalist>
          </div>
        </div>
      ...
      <div class="card-section">
        <div class="table-header">
          <h3 class="section-title">รายการอำเภอ</h3>
        ...
        <div v-else class="table-wrapper">
          <table class="data-table">
            <thead><tr><th>ชื่ออำเภอ</th><th>จังหวัด</th><th>สถานะ</th><th class="actions-col">จัดการ</th></tr></thead>
            <tbody>
              <tr v-if="districts.length === 0"><td colspan="4" class="empty-cell">ไม่พบข้อมูล</td></tr>
              <tr v-for="item in districts" :key="item.id">
                <td>{{ item.name }}</td>
                <td>{{ provinceMap[item.province_id] || '-' }}</td>
    ...
    <div v-if="isDeleteModalOpen" class="confirm-modal-overlay" @click="closeDeleteModal">
      <div class="confirm-modal" @click.stop>
        ...
        <p class="confirm-modal-text">คุณต้องการลบข้อมูลอำเภอนี้ใช่หรือไม่?</p>
