...
import type { GenderPayload } from '~/types/settings'
...
const form = reactive<GenderPayload>({
  name_th: '',
  name_en: '',
  is_active: true
})
...
function startEditRow(id: string) {
  const item = genders.value.find((gender) => gender.id === id)
  if (!item) {
    return
  }
...
async function handleSubmit() {
  ...
  const success = editingId.value
    ? await updateGender(editingId.value, payload)
...
async function handleDelete(id: string) {
  ...
  if (genders.value.length === 1 && currentPage.value > 1) {
...
<template>
  <div class="settings-page">
    ...
    <div class="page-header">
      <div>
        <h1 class="page-title">ตั้งค่าพื้นฐาน - เพศ</h1>
        <p class="page-subtitle">จัดการข้อมูลเพศสำหรับระบบสมาชิก</p>
      </div>
    </div>

    <div class="content-card">
      <form class="card-section" @submit.prevent="handleSubmit">
        <h3 class="section-title">{{ isEditing ? 'แก้ไขเพศ' : 'เพิ่มเพศ' }}</h3>
      ...
      <div class="card-section">
        <div class="table-header">
          <h3 class="section-title">รายการเพศ</h3>
        ...
        <div v-else class="table-wrapper">
          <table class="data-table">
            ...
            <tbody>
              <tr v-if="genders.length === 0">
                <td colspan="4" class="empty-cell">ไม่พบข้อมูล</td>
              </tr>
    ...
    <div v-if="isDeleteModalOpen" class="confirm-modal-overlay" @click="closeDeleteModal">
      <div class="confirm-modal" @click.stop>
        ...
        <p class="confirm-modal-text">คุณต้องการลบข้อมูลเพศนี้ใช่หรือไม่?</p>
