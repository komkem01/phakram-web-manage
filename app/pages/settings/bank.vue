...
import type { BankPayload } from '~/types/settings'

definePageMeta({ layout: 'dashboard' })

const { isLoading, errorMessage, successMessage, banks, paginate, fetchBanks, createBank, updateBank, deleteBank } = useSystemBanks()

const form = reactive<BankPayload>({ name_th: '', name_abb_th: '', name_en: '', name_abb_en: '', is_active: true })
...
function startEditRow(id: string) {
  const item = banks.value.find((bank) => bank.id === id)
  if (!item) return

  editingId.value = item.id
  form.name_th = item.name_th
  form.name_abb_th = item.name_abb_th
  form.name_en = item.name_en
  form.name_abb_en = item.name_abb_en
...
async function load(page = 1) {
  ...
  await fetchBanks({ page, size: pageSize.value, sort_by: 'created_at', order_by: 'desc' })
}
...
async function handleSubmit() {
  if (!form.name_th.trim() || !form.name_abb_th.trim() || !form.name_en.trim() || !form.name_abb_en.trim()) {
  ...
  const payload: BankPayload = {
    ...
    name_abb_th: form.name_abb_th.trim(),
    name_en: form.name_en.trim(),
    name_abb_en: form.name_abb_en.trim(),
    is_active: form.is_active
  }
  const success = editingId.value ? await updateBank(editingId.value, payload) : await createBank(payload)
...
<template>
  <div class="settings-page">
    ...
    <div class="page-header">
      <h1 class="page-title">ตั้งค่าพื้นฐาน - ธนาคาร</h1>
      <p class="page-subtitle">จัดการข้อมูลธนาคาร</p>
    </div>

    <div class="content-card">
      <form class="card-section" @submit.prevent="handleSubmit">
        <h3 class="section-title">{{ isEditing ? 'แก้ไขธนาคาร' : 'เพิ่มธนาคาร' }}</h3>

        <div class="form-grid">
          <div class="form-group">
            <label for="name_th">ชื่อธนาคาร (ไทย)</label>
            <input id="name_th" v-model="form.name_th" type="text" :disabled="isSubmitting" required>
          </div>
          <div class="form-group">
            <label for="name_abb_th">ชื่อย่อ (ไทย)</label>
            <input id="name_abb_th" v-model="form.name_abb_th" type="text" :disabled="isSubmitting" required>
          </div>
          <div class="form-group">
            <label for="name_en">ชื่อธนาคาร (อังกฤษ)</label>
            <input id="name_en" v-model="form.name_en" type="text" :disabled="isSubmitting" required>
          </div>
          <div class="form-group">
            <label for="name_abb_en">ชื่อย่อ (อังกฤษ)</label>
            <input id="name_abb_en" v-model="form.name_abb_en" type="text" :disabled="isSubmitting" required>
          </div>
        </div>
      ...
      <div class="card-section">
        <div class="table-header">
          <h3 class="section-title">รายการธนาคาร</h3>
        ...
        <div v-else class="table-wrapper">
          <table class="data-table">
            <thead><tr><th>ชื่อ (ไทย)</th><th>ชื่อย่อ (ไทย)</th><th>ชื่อ (อังกฤษ)</th><th>ชื่อย่อ (อังกฤษ)</th><th>สถานะ</th><th class="actions-col">จัดการ</th></tr></thead>
            <tbody>
              <tr v-if="banks.length === 0"><td colspan="6" class="empty-cell">ไม่พบข้อมูล</td></tr>
              <tr v-for="item in banks" :key="item.id">
                <td>{{ item.name_th }}</td>
                <td>{{ item.name_abb_th }}</td>
                <td>{{ item.name_en }}</td>
                <td>{{ item.name_abb_en }}</td>
    ...
    <div v-if="isDeleteModalOpen" class="confirm-modal-overlay" @click="closeDeleteModal">
      <div class="confirm-modal" @click.stop>
        ...
        <p class="confirm-modal-text">คุณต้องการลบข้อมูลธนาคารนี้ใช่หรือไม่?</p>
