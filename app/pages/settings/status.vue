...
const { isLoading, errorMessage, successMessage, statuses, paginate, fetchStatuses, createStatus, updateStatus, deleteStatus } = useSystemStatuses()

const form = reactive<StatusPayload>({ name_th: '', name_en: '', is_active: true })
...
function startEditRow(id: string) {
  const item = statuses.value.find((status) => status.id === id)
  if (!item) return
...
async function load(page = 1) {
  ...
  await fetchStatuses({ page, size: pageSize.value, sort_by: 'created_at', order_by: 'desc' })
}
...
async function handleSubmit() {
  ...
  const payload: StatusPayload = { name_th: form.name_th.trim(), name_en: form.name_en.trim(), is_active: form.is_active }
  const success = editingId.value ? await updateStatus(editingId.value, payload) : await createStatus(payload)
...
async function handleDelete(id: string) {
  ...
  if (!success) return
  if (statuses.value.length === 1 && currentPage.value > 1) {
...
<template>
  <div class="settings-page">
    ...
    <div class="page-header">
      <h1 class="page-title">ตั้งค่าพื้นฐาน - สถานะการใช้งาน</h1>
      <p class="page-subtitle">จัดการข้อมูลสถานะการใช้งาน</p>
    </div>

    <div class="content-card">
      <form class="card-section" @submit.prevent="handleSubmit">
        <h3 class="section-title">{{ isEditing ? 'แก้ไขสถานะการใช้งาน' : 'เพิ่มสถานะการใช้งาน' }}</h3>
      ...
      <div class="card-section">
        <div class="table-header">
          <h3 class="section-title">รายการสถานะการใช้งาน</h3>
        ...
        <div v-else class="table-wrapper">
          <table class="data-table">
            <thead><tr><th>ชื่อ (ไทย)</th><th>ชื่อ (อังกฤษ)</th><th>สถานะ</th><th class="actions-col">จัดการ</th></tr></thead>
            <tbody>
              <tr v-if="statuses.length === 0"><td colspan="4" class="empty-cell">ไม่พบข้อมูล</td></tr>
    ...
    <div v-if="isDeleteModalOpen" class="confirm-modal-overlay" @click="closeDeleteModal">
      <div class="confirm-modal" @click.stop>
        ...
        <p class="confirm-modal-text">คุณต้องการลบข้อมูลสถานะการใช้งานนี้ใช่หรือไม่?</p>
...
.status-pill { display: inline-flex; align-items: center; height: 26px; padding: 0 10px; border-radius: 999px; font-size: 12px; font-weight: 700; }
