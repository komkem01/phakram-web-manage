import type { ApiPaginate, ApiResponse, ListParams, SystemPaymentItem, SystemPaymentPayload } from '~/types/settings'
...
export function useSystemPayments() {
  ...
  const payments = ref<SystemPaymentItem[]>([])
  ...
  async function fetchPayments(params: ListParams = {}) {
    errorMessage.value = ''
    isLoading.value = true

    try {
      const response = await fetchWithAuthRetry<ApiResponse<SystemPaymentItem[]>>(buildUrl(params), { method: 'GET' })
      ...
      paginate.value = response.paginate || { page: 1, size: 10, total: payments.value.length }
      return true
    } catch (error) {
  ...
  async function createPayment(payload: SystemPaymentPayload) {
    errorMessage.value = ''
  ...
  async function updatePayment(id: string, payload: SystemPaymentPayload) {
    errorMessage.value = ''
    successMessage.value = ''
