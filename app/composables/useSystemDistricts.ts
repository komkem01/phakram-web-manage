import type { ApiPaginate, ApiResponse, DistrictItem, DistrictPayload, ListParams } from '~/types/settings'
import { SETTINGS_CONFIG } from '~/constants/settings'

export function useSystemDistricts() {
  ...
  function buildUrl(params: ListParams = {}) {
    ...
    const endpoint = SETTINGS_CONFIG.endpoints.districts
  ...
  async function fetchDistricts(params: ListParams = {}) {
    errorMessage.value = ''
    isLoading.value = true

    try {
      const response = await fetchWithAuthRetry<ApiResponse<DistrictItem[]>>(buildUrl(params), { method: 'GET' })
      ...
      paginate.value = response.paginate || { page: 1, size: 10, total: districts.value.length }
      return true
    } catch (error) {
  ...
  async function createDistrict(payload: DistrictPayload) {
    errorMessage.value = ''
  ...
  async function updateDistrict(id: string, payload: DistrictPayload) {
    errorMessage.value = ''
    successMessage.value = ''
