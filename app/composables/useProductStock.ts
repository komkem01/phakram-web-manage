import type { ApiResponse, ProductStockItem, ProductStockPayload } from '~/types/settings'
...
export function useProductStock() {
  ...
  const stock = ref<ProductStockItem | null>(null)
  ...
  function buildPath(productId: string) {
    ...
    return `${apiBaseUrl}${SETTINGS_CONFIG.endpoints.products}/${productId}/stock`
  }
  ...
  async function fetchStock(productId: string) {
    ...
    errorMessage.value = ''
    try {
      const response = await fetchWithAuthRetry<ApiResponse<ProductStockItem>>(buildPath(productId), { method: 'GET' })
      if (!isSuccessCode(response.code)) throw new Error(response.message)
      stock.value = response.data || null
      return true
    } catch {
      stock.value = null
      return false
  ...
  async function createStock(productId: string, payload: ProductStockPayload) {
    errorMessage.value = ''
  ...
  async function updateStock(productId: string, payload: ProductStockPayload) {
    errorMessage.value = ''
    successMessage.value = ''
    try {
  ...
  return { isLoading, errorMessage, successMessage, stock, fetchStock, createStock, updateStock, deleteStock }
}
