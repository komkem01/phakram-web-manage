import type { ApiPaginate, ApiResponse, ListParams, ProvinceItem, ProvincePayload } from '~/types/settings'
import { SETTINGS_CONFIG } from '~/constants/settings'

export function useSystemProvinces() {
  ...
  function buildUrl(params: ListParams = {}) {
    ...
    const endpoint = SETTINGS_CONFIG.endpoints.provinces
  ...
  async function fetchProvinces(params: ListParams = {}) {
    errorMessage.value = ''
    isLoading.value = true

    try {
      const response = await fetchWithAuthRetry<ApiResponse<ProvinceItem[]>>(buildUrl(params), { method: 'GET' })
      ...
      paginate.value = response.paginate || { page: 1, size: 10, total: provinces.value.length }
      return true
    } catch (error) {
  ...
  async function createProvince(payload: ProvincePayload) {
    errorMessage.value = ''
  ...
  async function updateProvince(id: string, payload: ProvincePayload) {
    errorMessage.value = ''
    successMessage.value = ''
