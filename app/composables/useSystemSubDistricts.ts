import type { ApiPaginate, ApiResponse, ListParams, SubDistrictItem, SubDistrictPayload } from '~/types/settings'
import { SETTINGS_CONFIG } from '~/constants/settings'

export function useSystemSubDistricts() {
  ...
  function buildUrl(params: ListParams = {}) {
    ...
    const endpoint = SETTINGS_CONFIG.endpoints.subDistricts
  ...
  async function fetchSubDistricts(params: ListParams = {}) {
    errorMessage.value = ''
    isLoading.value = true

    try {
      const response = await fetchWithAuthRetry<ApiResponse<SubDistrictItem[]>>(buildUrl(params), { method: 'GET' })
      ...
      paginate.value = response.paginate || { page: 1, size: 10, total: subDistricts.value.length }
      return true
    } catch (error) {
  ...
  async function createSubDistrict(payload: SubDistrictPayload) {
    errorMessage.value = ''
  ...
  async function updateSubDistrict(id: string, payload: SubDistrictPayload) {
    errorMessage.value = ''
    successMessage.value = ''
  ...
  async function deleteSubDistrict(id: string) {
    errorMessage.value = ''
    successMessage.value = ''
