import type { ApiPaginate, ApiResponse, GenderItem, GenderPayload, ListParams } from '~/types/settings'
import { SETTINGS_CONFIG } from '~/constants/settings'

export function useSystemGenders() {
  ...
  function buildUrl(params: ListParams = {}) {
    ...
    const endpoint = SETTINGS_CONFIG.endpoints.genders
  ...
  async function fetchGenders(params: ListParams = {}) {
    errorMessage.value = ''
    isLoading.value = true

    try {
      const response = await fetchWithAuthRetry<ApiResponse<GenderItem[]>>(buildUrl(params), {
        method: 'GET'
      })
      ...
      paginate.value = response.paginate || { page: 1, size: 10, total: genders.value.length }
      return true
    } catch (error) {
  ...
  async function createGender(payload: GenderPayload) {
    errorMessage.value = ''
  ...
  async function updateGender(id: string, payload: GenderPayload) {
    errorMessage.value = ''
    successMessage.value = ''
