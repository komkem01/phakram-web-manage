...
export function useSystemPromotions() {
  ...
  function buildUrl(params: PromotionListParams = {}) {
    const apiBaseUrl = (config.public.apiBaseUrl as string | undefined)?.replace(/\/$/, '') || ''
    const endpoint = SETTINGS_CONFIG.endpoints.promotions
  ...
  async function fetchPromotions(params: PromotionListParams = {}) {
    errorMessage.value = ''
    isLoading.value = true

    try {
      const response = await fetchWithAuthRetry<ApiResponse<PromotionItem[]>>(buildUrl(params), { method: 'GET' })
      ...
      paginate.value = response.paginate || { page: 1, size: 10, total: promotions.value.length }
      return true
    } catch (error) {
  ...
  async function updatePromotion(id: string, payload: PromotionPayload) {
    errorMessage.value = ''
    successMessage.value = ''
