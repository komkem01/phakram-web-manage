import type { ApiPaginate, ApiResponse, CategoryItem, CategoryPayload, ListParams } from '~/types/settings'
...
export function useSystemCategories() {
  ...
  function buildUrl(params: ListParams = {}) {
    ...
    const endpoint = SETTINGS_CONFIG.endpoints.categories
  ...
  async function fetchCategories(params: ListParams = {}) {
    ...
    try {
      const response = await fetchWithAuthRetry<ApiResponse<CategoryItem[]>>(buildUrl(params), { method: 'GET' })
      ...
      paginate.value = response.paginate || { page: 1, size: 10, total: categories.value.length }
      return true
    } catch (error) {
  ...
  async function createCategory(payload: CategoryPayload) {
    ...
    try {
      ...
      const response = await fetchWithAuthRetry<ApiResponse<null>>(buildPath(), { method: 'POST', body })
  ...
  async function updateCategory(id: string, payload: CategoryPayload) {
    ...
    try {
      ...
      const response = await fetchWithAuthRetry<ApiResponse<null>>(buildPath(id), { method: 'PATCH', body })
