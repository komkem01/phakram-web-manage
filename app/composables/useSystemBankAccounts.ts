import type { ApiPaginate, ApiResponse, ListParams, SystemBankAccountItem, SystemBankAccountPayload } from '~/types/settings'
import { SETTINGS_CONFIG } from '~/constants/settings'

export function useSystemBankAccounts() {
  ...
  const bankAccounts = ref<SystemBankAccountItem[]>([])
  ...
  function buildUrl(params: ListParams = {}) {
    ...
    const endpoint = SETTINGS_CONFIG.endpoints.systemBankAccounts
  ...
  async function fetchSystemBankAccounts(params: ListParams = {}) {
    errorMessage.value = ''
    isLoading.value = true

    try {
      const response = await fetchWithAuthRetry<ApiResponse<SystemBankAccountItem[]>>(buildUrl(params), { method: 'GET' })
      ...
      paginate.value = response.paginate || { page: 1, size: 10, total: bankAccounts.value.length }
      return true
    } catch (error) {
  ...
  async function createSystemBankAccount(payload: SystemBankAccountPayload) {
    errorMessage.value = ''
  ...
  async function updateSystemBankAccount(id: string, payload: SystemBankAccountPayload) {
    errorMessage.value = ''
    successMessage.value = ''
  ...
  async function deleteSystemBankAccount(id: string) {
    errorMessage.value = ''
    successMessage.value = ''
