import type { ApiPaginate, ApiResponse, ListParams, ZipcodeItem, ZipcodePayload } from '~/types/settings'
import { SETTINGS_CONFIG } from '~/constants/settings'

export function useSystemZipcodes() {
  ...
  function buildUrl(params: ListParams = {}) {
    ...
    const endpoint = SETTINGS_CONFIG.endpoints.zipcodes
  ...
  async function fetchZipcodes(params: ListParams = {}) {
    errorMessage.value = ''
    isLoading.value = true

    try {
      const response = await fetchWithAuthRetry<ApiResponse<ZipcodeItem[]>>(buildUrl(params), { method: 'GET' })

      if (!isSuccessCode(response.code)) {
        throw new Error(response.message || SETTINGS_CONFIG.messages.loadError)
      }

      zipcodes.value = response.data || []
      paginate.value = response.paginate || { page: 1, size: 10, total: zipcodes.value.length }
      return true
    } catch (error) {
  ...
  async function createZipcode(payload: ZipcodePayload) {
    errorMessage.value = ''
  ...
  async function updateZipcode(id: string, payload: ZipcodePayload) {
    errorMessage.value = ''
    successMessage.value = ''
  ...
  async function deleteZipcode(id: string) {
    errorMessage.value = ''
    successMessage.value = ''
