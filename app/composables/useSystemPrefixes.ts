import type { ApiPaginate, ApiResponse, ListParams, PrefixItem, PrefixPayload } from '~/types/settings'
import { SETTINGS_CONFIG } from '~/constants/settings'

export function useSystemPrefixes() {
  ...
  function buildUrl(params: ListParams = {}) {
    ...
    const endpoint = SETTINGS_CONFIG.endpoints.prefixes
  ...
  async function fetchPrefixes(params: ListParams = {}) {
    errorMessage.value = ''
    isLoading.value = true

    try {
      const response = await fetchWithAuthRetry<ApiResponse<PrefixItem[]>>(buildUrl(params), {
        method: 'GET'
      })
      ...
      paginate.value = response.paginate || { page: 1, size: 10, total: prefixes.value.length }
      return true
    } catch (error) {
      if (error && typeof error === 'object' && 'data' in error) {
        ...
        errorMessage.value = error instanceof Error
          ? error.message
          : SETTINGS_CONFIG.messages.loadError
      }
      return false
    } finally {
  ...
  async function createPrefix(payload: PrefixPayload) {
    errorMessage.value = ''
    successMessage.value = ''

    try {
      const response = await fetchWithAuthRetry<ApiResponse<null>>(buildPath(), {
        method: 'POST',
        body: payload
      })
      ...
      if (error && typeof error === 'object' && 'data' in error) {
        ...
        errorMessage.value = error instanceof Error
          ? error.message
          : SETTINGS_CONFIG.messages.createError
      }
      return false
    }
  }

  async function updatePrefix(id: string, payload: PrefixPayload) {
    errorMessage.value = ''
    successMessage.value = ''
    ...
    try {
      ...
      if (error && typeof error === 'object' && 'data' in error) {
        ...
        errorMessage.value = error instanceof Error
          ? error.message
          : SETTINGS_CONFIG.messages.updateError
      }
      return false
    }
  }
  ...
  async function deletePrefix(id: string) {
    ...
    try {
      ...
      if (error && typeof error === 'object' && 'data' in error) {
        ...
        errorMessage.value = error instanceof Error
          ? error.message
          : SETTINGS_CONFIG.messages.deleteError
      }
      return false
    }
  }
