import type { ApiPaginate, ApiResponse, BankItem, BankPayload, ListParams } from '~/types/settings'
import { SETTINGS_CONFIG } from '~/constants/settings'

export function useSystemBanks() {
  ...
  async function fetchBanks(params: ListParams = {}) {
    errorMessage.value = ''
    isLoading.value = true

    try {
      const response = await fetchWithAuthRetry<ApiResponse<BankItem[]>>(buildUrl(params), { method: 'GET' })
      ...
      paginate.value = response.paginate || { page: 1, size: 10, total: banks.value.length }
      return true
    } catch (error) {
  ...
  async function createBank(payload: BankPayload) {
    errorMessage.value = ''
  ...
  async function updateBank(id: string, payload: BankPayload) {
    errorMessage.value = ''
    successMessage.value = ''
