import type { ApiPaginate, ApiResponse, ListParams, ProductItem, ProductPayload } from '~/types/settings'
...
export function useSystemProducts() {
  ...
  async function fetchProducts(params: ListParams = {}) {
    errorMessage.value = ''
    isLoading.value = true

    try {
      const response = await fetchWithAuthRetry<ApiResponse<ProductItem[]>>(buildUrl(params), { method: 'GET' })
      ...
      paginate.value = response.paginate || { page: 1, size: 10, total: products.value.length }
      return true
    } catch (error) {
  ...
  async function fetchProductById(id: string) {
    ...
    try {
      const response = await fetchWithAuthRetry<ApiResponse<ProductItem>>(buildPath(id), { method: 'GET' })
